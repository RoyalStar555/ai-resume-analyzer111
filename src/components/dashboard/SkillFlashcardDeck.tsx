  ...
  return (
    <div className="glass rounded-2xl p-5 shadow-card">
      <div className="flex items-center justify-between gap-3">
        ...
        <span className="font-mono text-xs text-muted-foreground">{(index % skills.length) + 1}/{skills.length}</span>
      </div>
      <div className="mt-5 min-h-28 rounded-xl border border-primary/30 bg-primary/10 p-5">
        ...
        <p className="mt-2 text-xs text-muted-foreground">Add proof, outcomes, or a project example for this skill.</p>
      </div>
      <div className="mt-4 flex justify-between">
        <Button size="icon" variant="outline" onClick={() => setIndex((value) => (value - 1 + skills.length) % skills.length)} aria-label="Previous skill">
          <ArrowLeft />
        </Button>
        <Button size="icon" variant="outline" onClick={() => setIndex((value) => (value + 1) % skills.length)} aria-label="Next skill">
          <ArrowRight />
        </Button>
      </div>
    </div>
